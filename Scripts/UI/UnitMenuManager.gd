...
@onready var active_menu = main_menu : set = set_active_menu
...
func _ready():
	...
	for button_name in main_menu_buttons:
		if mode == 0:
			...
			mode = 1 if button_name == "Overcharge" else 0
		else:
			create_menu_button(button_name, main_menu_buttons[button_name], main_menu)
			mode = 0 if button_name == "Protocol" else 1
	
	create_menu_button("Return", back_button, attack_menu)
	for button_name in attack_menu_buttons:
		create_action_button(button_name, attack_menu_buttons[button_name], attack_menu)
	
	create_menu_button("Return", back_button, tech_menu)
	for button_name in tech_menu_buttons:
		create_action_button(button_name, tech_menu_buttons[button_name], tech_menu)
	
	create_menu_button("Return", back_button, system_menu)
	for button_name in system_menu_buttons:
		create_action_button(button_name, system_menu_buttons[button_name], system_menu)
	
	create_menu_button("Return", back_button, reaction_menu)
	for button_name in reaction_menu_buttons:
		create_action_button(button_name, reaction_menu_buttons[button_name], reaction_menu)
	
	create_menu_button("Return", back_button, protocol_menu)
	for button_name in protocol_menu_buttons:
		create_action_button(button_name, protocol_menu_buttons[button_name], protocol_menu)



func create_action_button(button_name: String, button_dict: Dictionary, menu: HBoxContainer):
...
func create_menu_button(button_name: String, button_dict: Dictionary, menu: HBoxContainer):
